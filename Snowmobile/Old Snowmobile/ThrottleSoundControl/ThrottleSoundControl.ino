 ...
 */
 ...
 if (digitalRead(button) == HIGH){
  digitalWrite(startSound, true);
 }
 else
 {
  digitalWrite(startSound, false);
 }
 int l;
  /*for(l = 0; l<500; l++){
    int k = 200;
    currentSmpleRate = F_CPU / (BASE_RATE + long(k * TOP_SPEED_MULTIPLIER));
    k++;
    delay(.5);
  }
  */
...
void manageSpeed(){
  ...
  if(millis() - throtMillis > 5) {
    ...
    if(currentThrottle +5 > currentRpm){
      currentRpm += 4;
      if(currentRpm > maxRpm) currentRpm = maxRpm;
      prevThrottle = currentThrottle;

    }
    else if(currentThrottle -7 < currentRpm){
      currentRpm -= 6;
